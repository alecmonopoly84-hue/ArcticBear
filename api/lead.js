...
function normalize(s={}){s=s&&typeof s==='object'?s:{};const pk=periodKeys(),km={d:pk.d,w:pk.w,mo:pk.mo};for(const p of['d','w','mo']){if(!s[p]||typeof s[p]!=='object'||s[p].k!==km[p])s[p]={k:km[p]};for(const k of['n','s','p','t','c','x','y','f','na','rs','rc'])if(!Number.isFinite(s[p][k]))s[p][k]=0;s[p].m=s[p].m&&typeof s[p].m==='object'?s[p].m:{};s[p].r=s[p].r&&typeof s[p].r==='object'?s[p].r:{}}s.a=Array.isArray(s.a)?s.a.slice(0,MAX):[];s.v=4;return s}
function mgr(st,n){st.m[n]=st.m[n]||{t:0,c:0,x:0,y:0,f:0,na:0};return st.m[n]}
function apply(st,e){if(e.type==='new'){st.n++;if(e.kind==='parts')st.p++;else st.s++;return}const b=mgr(st,e.manager||'Сотрудник');if(e.type==='take'){st.t++;b.t++}if(e.type==='no_answer'){st.na++;b.na++}if(e.type==='contacted'){st.c++;b.c++;if(Number.isFinite(e.response)){st.rs+=Math.round(e.response);st.rc++}}if(e.type==='success'){st.y++;st.x++;b.y++;b.x++}if(e.type==='failed'){st.f++;st.x++;b.f++;b.x++;st.r[e.reason||'Другое']=(st.r[e.reason||'Другое']||0)+1}}
function applyActive(s,e){if(e.type==='new'&&e.lead){s.a=[e.lead,...s.a.filter(x=>String(x.i)!==String(e.lead.i))].slice(0,MAX);return}if(!e.id)return;const i=s.a.findIndex(x=>String(x.i)===String(e.id));if(i<0)return;if(['success','failed'].includes(e.type)){s.a.splice(i,1);return}if(e.manager)s.a[i].o=e.manager.slice(0,28);if(e.user)s.a[i].u=e.user;if(e.status)s.a[i].st=e.status}
...
const phone=v=>{const r=String(v||'').trim(),d=r.replace(/\D/g,'');if(d.length===11&&d[0]==='8')return'+7'+d.slice(1);if(d.length===11&&d[0]==='7')return'+'+d;return r};
const safe=v=>(String(v||'attachment').replace(/[\\/\0\r\n]/g,'_').trim().slice(0,100)||'attachment');
async function attachment(token,a,caption){const type=String(a.type||'application/octet-stream'),img=type.startsWith('image/'),method=img?'sendPhoto':'sendDocument',field=img?'photo':'document',bytes=Buffer.from(String(a.data||''),'base64');if(!bytes.length||bytes.length>MAX_BYTES)return{ok:false,error:'file too large'};const f=new FormData();f.append('chat_id',CHAT);f.append(field,new Blob([bytes],{type}),safe(a.name));f.append('caption',caption);const r=await fetch(`https://api.telegram.org/bot${token}/${method}`,{method:'POST',body:f}),d=await r.json();return{ok:r.ok&&!!d.ok,error:d.description||null}}
async function submitLead(token,b){await install(token);const ph=phone(b.phone);if(ph.replace(/\D/g,'').length<5)return{ok:false,status:400,error:'Phone is required'};const kind=b.kind==='parts'?'parts':'service',parts=kind==='parts',label=parts?'ЗАПЧАСТИ':'СЕРВИС',icon=parts?'🧩':'🛠',aa=Array.isArray(b.attachments)?b.attachments.slice(0,MAX_ATTACH):[];const lines=[`${icon} <b>НОВАЯ ЗАЯВКА · ${label}</b>`,`<i>ABService · ${now()}</i>`,'',b.name?`👤 <b>Клиент:</b> ${esc(b.name)}`:null,`📞 <b>Телефон:</b> ${esc(ph)}`,b.machine?`🚜 <b>Техника:</b> ${esc(b.machine)}`:null,!parts&&b.location?`📍 <b>Локация:</b> ${esc(b.location)}`:null,!parts&&b.issue?`⚠️ <b>Проблема:</b> ${esc(b.issue)}`:null,parts&&b.mode?`🔧 <b>Формат:</b> ${esc(b.mode)}`:null,parts&&b.article?`🏷 <b>Артикул:</b> ${esc(b.article)}`:null,parts&&b.part?`📦 <b>Запчасть:</b> ${esc(b.part)}`:null,aa.length?`📷 <b>Вложений:</b> ${aa.length}`:null,'',`🌐 <b>Источник:</b> ${parts?'ABService · Запчасти':'ABService · Сервис'}`].filter(x=>x!==null);const sent=await tg(token,'sendMessage',{chat_id:CHAT,text:`${lines.join('\n')}${SEP}🔵 <b>Статус:</b> НОВАЯ`,parse_mode:'HTML',disable_web_page_preview:true,reply_markup:keys('new')});if(!sent.ok)return{ok:false,status:502,error:sent.data?.description||'Telegram failed'};const id=sent.data.result.message_id,c=await getCrm(token);if(c.ok){const lead={i:id,k:parts?'p':'s',st:'new',u:'',o:'',q:String(b.name||b.machine||b.part||ph).replace(/\s+/g,' ').trim().slice(0,28)};await saveCrm(token,c.id,record(c.state,{type:'new',kind,lead}))}let sentFiles=0,errs=[];for(let i=0;i<aa.length;i++){const r=await attachment(token,aa[i],`📎 ${label} · вложение ${i+1}/${aa.length} · ${ph}`);if(r.ok)sentFiles++;else errs.push(r.error||'attachment failed')}return{ok:true,status:200,attachmentsRequested:aa.length,attachmentsSent:sentFiles,attachmentErrors:errs}}
